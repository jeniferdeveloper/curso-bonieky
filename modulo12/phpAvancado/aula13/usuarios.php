...
class Usuarios {
    ...
    public function selecionar($id) {
        ...
        $array = array();
        //se houve retorno de algum resultado entao eu vou preencher o array
        if($sql->rowCount() > 0){
            //fetch só traz um item
            $array = $sql->fetch();
        }
    ...
    public function atualizar($nome, $email, $senha, $id){
        $sql = $this->db->prepare("UPDATE INTO usuarios SET nome = ?, email = ?, senha = ? WHERE id = ?");
